...
SELECT title FROM movie JOIN casting ON movie.id=movieid JOIN actor ON actor.id=actorid WHERE ord != 1 AND name = 'Harrison Ford' 

/* 10. List the films together with the leading star for all 1962 films.
*/

SELECT distinct title,name FROM movie JOIN casting ON movie.id=movieid JOIN actor ON actor.id=actorid WHERE yr =1962 AND ord=1
